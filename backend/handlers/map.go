...
func fetchMapsForScope(gameID, chapterID int) ([]models.MapSelect, error) {
	rows, err := database.DB.Query(`
		SELECT
			...
			COALESCE(cat.id, 0), COALESCE(cat.name, ''), COALESCE(best.score_count, 0)
		FROM maps m
		INNER JOIN games g ON g.id = m.game_id
		INNER JOIN chapters c ON c.id = m.chapter_id
		LEFT JOIN game_categories gc ON gc.game_id = m.game_id
		LEFT JOIN categories cat ON cat.id = gc.category_id
		LEFT JOIN (
	...
	for rows.Next() {
		...
		if hasMap && mapData.ID == lastMapID {
			if categoryPortal.Category.ID != 0 {
				maps[len(maps)-1].CategoryPortals = append(maps[len(maps)-1].CategoryPortals, categoryPortal)
			}
			continue
		}
		mapData.CategoryPortals = []models.CategoryPortal{}
		if categoryPortal.Category.ID != 0 {
			mapData.CategoryPortals = append(mapData.CategoryPortals, categoryPortal)
		}
