...
const ModMenu: React.FC<ModMenuProps> = ({
  token,
  data,
  selectedRun,
  mapID,
  ...
  const _delete_map_summary_route = async () => {
    if (
      ...
      if (token) {
        const success = await API.delete_map_summary(
          token,
          mapID,
          data.summary.routes[selectedRun].route_id
        );
        if (success) {
          navigate(0);
        } else {
    ...
    <>
      ...
      <div id="modview">
        <div>
          ...
          <button onClick={() => _delete_map_summary_route()}>
              ...
              <div>
                <span>
                  ...
                  <input
                    ...
                    onChange={e => {
                      if (e.target.files) {
                        compressImage(e.target.files[0]).then(d => setImage(d));
                      }
                    }}
                  />
                </span>
                {image ? (
                  <button onClick={() => _edit_map_summary_image()}>
                    upload
                  </button>
                ) : (
                  <span></span>
          ...
          menu === 2 && (
            <div id="modview-menu-edit">
              ...
              >
                ...
                <textarea
                  value={routeContent.description}
                  onChange={e => {
                    setRouteContent({
                      ...routeContent,
