...
const Sidebar: React.FC<SidebarProps> = ({ setToken, profile, setProfile, onUploadRun }) => {
  ...
  React.useEffect(() => {
	  const btns = document.querySelectorAll("#sidebarBtn");
	  btns.forEach((e, num) => {
		    e.setAttribute("num", num.toString());
	  });
  })
    ...
    <div id='sidebar'>
      ...
      <div id='sidebar-list'> {/* List */}
        <div id='sidebar-toplist'> {/* Top */}
          ...
          <button id="sidebarBtn" className='sidebar-button' onClick={() => _handle_sidebar_lock()}><img src={SearchIcon} alt="" /><span>Search</span></button>

          <span></span>

          <Link to="/" tabIndex={-1}>
			  <button ref={btnRef} id="sidebarBtn" className={`${btn.sidebar}`}><img src={HomeIcon} alt="homepage" /><span>Home&nbsp;Page</span></button>
          </Link>

          <Link to="/games" tabIndex={-1}>
            <button id="sidebarBtn" className='sidebar-button'><img src={PortalIcon} alt="games" /><span>Games</span></button>
          </Link>

          <Link to="/rankings" tabIndex={-1}>
            <button id="sidebarBtn" className='sidebar-button'><img src={FlagIcon} alt="rankings" /><span>Rankings</span></button>
          </Link>
        ...
        <div id='sidebar-bottomlist'>
          ...
          <Link to="/rules" tabIndex={-1}>
            <button id="sidebarBtn" className='sidebar-button'><img src={BookIcon} alt="rules" /><span>Leaderboard&nbsp;Rules</span></button>
          </Link>

          <Link to="/about" tabIndex={-1}>
            <button id="sidebarBtn" className='sidebar-button'><img src={HelpIcon} alt="about" /><span>About&nbsp;LPHUB</span></button>
          </Link>
        </div>
      </div>
      <div>
