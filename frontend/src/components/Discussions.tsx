...
const Discussions: React.FC<DiscussionsProps> = ({
  ...
  const _create_map_discussion_comment = async (discussion_id: number) => {
    if (token) {
      await API.post_map_discussion_comment(
        token,
        mapID,
        discussion_id,
        createDiscussionCommentContent
      );
  ...
  return (
    <section id="section7" className="summary3">
      ...
      <div id="discussion-search">
        ...
        />
        <div>
          <button onClick={() => setCreateDiscussion(true)}>New Post</button>
        </div>
      </div>

      {
        ...
        createDiscussion ? (
          ...
          <div id="discussion-thread">
            ...
            <div>
              <Link
                to={`/users/${discussionThread.discussion.creator.steam_id}`}
              >
                <img
                  src={discussionThread.discussion.creator.avatar_link}
                  alt=""
                />
              </Link>
              <div>
            ...
            <div id="discussion-send">
              <input
                ...
                onChange={e =>
                  setCreateDiscussionCommentContent(e.target.value)
                }
              />
              <div>
                <button
          ...
          <>
            {data.discussions
              .filter(f => f.title.includes(discussionSearch))
              .sort(
                (a, b) =>
